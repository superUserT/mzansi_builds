...
export default function Profile() {
  ...
  const handleCompleteProject = async (projectId: string) => {
    ...
    try {
      ...
      loadProjects();
      toast.success("Project Completed! Check the Wall of Fame.", {
        id: toastId,
      });
    } catch (error: any) {
  ...
  const addSupportRequirement = () => {
    if (
      supportInput.trim() &&
      !newProject.supportRequired?.includes(supportInput.trim())
    ) {
      setNewProject({
        ...newProject,
        supportRequired: [
          ...(newProject.supportRequired || []),
          supportInput.trim(),
        ],
      });
      setSupportInput("");
        ...
        {projects.filter((p) => !p.isCompleted).length === 0 ? (
          <Grid size={{ xs: 12 }}>
            <Typography color="textSecondary">
              No active projects. Start building!
            </Typography>
          </Grid>
        ) : (
          projects
      ...
      >
        ...
        <DialogContent dividers sx={{ borderColor: "#27272A" }}>
          ...
          <TextField
            ...
            onChange={(e) =>
              setNewProject({ ...newProject, stage: e.target.value as any })
            }
          >
            {STAGES.map((option) => (
          ...
          <Box display="flex" gap={1} alignItems="center" mt={2}>
            <TextField
              fullWidth
              label="Support Required (e.g., UI/UX, QA)"
              size="small"
              value={supportInput}
              onChange={(e) => setSupportInput(e.target.value)}
              onKeyDown={(e) =>
                e.key === "Enter" &&
                (e.preventDefault(), addSupportRequirement())
              }
            />
            <Button
              variant="outlined"
            ...
            {newProject.supportRequired?.map((req, idx) => (
              <Chip
                ...
                onDelete={() =>
                  setNewProject({
                    ...newProject,
                    supportRequired: newProject.supportRequired?.filter(
                      (_, i) => i !== idx,
                    ),
                  })
                }
        ...
        <DialogActions sx={{ p: 2 }}>
          <Button onClick={() => setOpenProjectModal(false)} color="inherit">
            Cancel
          </Button>
          <Button
      ...
      >
        ...
        <DialogContent dividers sx={{ borderColor: "#27272A" }}>
          <Typography variant="body2" color="textSecondary" mb={2}>
            What did you accomplish? This will be broadcast to the live
            community feed!
          </Typography>
          <TextField
            fullWidth
        ...
        <DialogActions sx={{ p: 2 }}>
          <Button onClick={() => setOpenMilestoneModal(false)} color="inherit">
            Cancel
          </Button>
          <Button
            ...
            variant="contained"
            color="primary"
            disabled={!milestoneDesc || loading}
          >
            Post Update
          </Button>
        </DialogActions>
